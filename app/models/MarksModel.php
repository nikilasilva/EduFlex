...
class MarksModel {
    ...
    public function insertMarks($studentId, $subjectId, $term, $marks, $classId) {
        ...
        if ($existing) {
            ...
            $this->query("UPDATE marks SET marks = :marks, class_id = :classid WHERE student_id = :sid AND subject_id = :subid AND term = :term", [
                ':sid' => $studentId,
                ':subid' => $subjectId,
                ':term' => $term,
                ':marks' => $marks,
            ...
            $this->query("INSERT INTO marks (student_id, subject_id, term, marks, class_id) VALUES (:sid, :subid, :term, :marks, :classid)", [
                ':sid' => $studentId,
                ':subid' => $subjectId,
                ':term' => $term,
                ':marks' => $marks,
    ...
    public function getClassReport($classId, $term) {
        $query = "
            SELECT s.student_id, s.name AS student_name, sub.name AS subject_name, m.marks
            FROM marks m
            JOIN students s ON m.student_id = s.student_id
            JOIN subjects sub ON m.subject_id = sub.id
            WHERE s.class_id = :class_id AND m.term = :term
        ";
    ...
    public function getStudentRanks($classId, $term) {
        $query = "
            SELECT s.student_id, s.name AS student_name, AVG(m.marks) AS average_marks
            FROM students s
    ...
    public function getClassReportByTerm($classId, $term)
{
    $sql = "
        ...
        WHERE m.class_id = :classId AND m.term = :term
    ...
    public function getStudentRanksByTerm($classId, $term) {
        $query = "
            ...
            ORDER BY average_marks DESC
        ";
        return $this->query($query, [':class_id' => $classId, ':term' => $term]);
    }
